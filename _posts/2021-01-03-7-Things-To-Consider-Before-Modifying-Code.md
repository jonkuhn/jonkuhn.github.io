...
## Does the code being modified have good unit tests?
Whether an existing class is being modified directly to add new behavior or it is being refactored, it is important to review the unit tests that exist for the class.  You can be more confident in modifying a class that already has a good suite of understandable unit tests, because the tests will help catch unintended changes in behavior.

If the tests are difficult to understand or do not cover the behavior of the class well, it is a good idea to improve the tests before making any kind of changes.

If you are refactoring the class to split something out, the tests you add should at least cover the responsibility that is being split out, but it is also good to consider covering related functionality that looks like it could accidentally be broken during the refactor.  The goal here is not to have perfect unit tests for the existing class.  The goal is just to make an incremental improvement to the tests and to be confident in being able to refactor the code without breaking anything.

Even though the unit tests covering the responsibility being split out are likely to end up in a test suite for the new class after the refactor, it can be a good idea to first write unit tests for this responsibility against the existing class before refactoring.  One advantage of doing this is that the act of writing the tests will help deepen your understanding of the responsibility that is being split out.  Another reason is that it is often possible to run these same tests before and after the class is split out in order to verify everything works the same.  

If you will be modifying a class to directly add new behavior to it, the unit tests you add should cover any behavior likely to be affected by the modifications.

If you are modifying a class to fix a bug, it is a good idea to first write a failing unit test that exposes the bug and then implement the fix to make the test pass.
